...
outer radar update
  ...
  -> IsSpectatorCheck (icon colour gate)
       -> return false only inside the POV scope  → enter live colour branch
  -> GetEntityBySlot(0 / -1) as "local" controller
       -> remap to observed player slot  → team 2/3 so competitive RGB paints
...
| GetEntityBySlot (local controller) | RVA `0x926920` | Icon renderer hardcodes `GetEntityBySlot(0)` as local. Demo slot 0 is the freecam spectator (team 1), so live colour paint's `FUN_180848f80(local)==2/3` gate never fires. During POV remap `0`/`-1` → observed player slot. Resolve via `xor ecx,ecx; call X; …; call IsSpectatorCheck` — do not blindly take every `cmp ecx,-1` / `call [rax+310h]` helper (near-identical array lookups exist). |
| Per-player icon renderer | RVA `0xE460E0` | Called from player update; branches on IsSpectatorCheck + demo state into team-colour vs per-player colour paths. Live RGB also needs local team 2/3 via GetEntityBySlot. |
...
- install line includes `isSpecCheck=1` and `getEntityBySlot=1`;
- `IsSpectatorCheck @ ...` and `GetEntityBySlot @ ...` appear once;
- after POV: `IsSpectatorCheck forced 0`, `GetEntityBySlot 0 -> observed slot N`;
